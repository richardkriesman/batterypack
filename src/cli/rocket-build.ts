...
asSubcommand(
  async (project, opts) => {
    ...
    // compile the project
    const artifact: Artifact = await withUiContext<Artifact>(
      "Compiling project",
      async () => {
        return unit.build();
      },
    ...
    // optionally, bundle the compiled code into a single executable
    if (opts.bundle) {
      await withUiContext("Bundling artifacts", async () => {
        await artifact.bundle();
      });
    }
  ...
  (command) => {
    command.option("--bundle", "combine build artifacts into a single file");
  }
);
